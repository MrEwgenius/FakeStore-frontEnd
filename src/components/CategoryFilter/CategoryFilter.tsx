...
const CategoryFilter: FC<CategoryFilterProps> = ({
    ...
    const CustomToggle: FC<{ children: ReactNode; eventKey: string }> = ({
        children,
        eventKey,
    }) => {
        ...
        return (
            <div
                className={classNames(style.customToggle)}
    ...
    return (
        <Accordion>
            <Accordion.Item eventKey="0">
                <Accordion.Header className={style.NameFilter}>
                    {selectedCategory ? selectedCategory : t("clothes")}
                </Accordion.Header>
                {typeProducts.map((type) => (
                    <Accordion.Body
                        key={type.id}
                        onClick={() => handleCategoryClick(type.name)}
                        className={style.bodyFilter}
                    >
