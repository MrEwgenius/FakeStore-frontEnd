...
const getSearchProducts = (limit?: number, page?: number, search?: string) => {
...
const addSubscribeInner = (data: addSubscribeUser, token: string) => {
    return API.post(`/user/usersubscribe`, data, {
        headers: {
            Authorization: `Bearer ${token}`,
        },
    });
